...
class fmgApi5:
        ...
        def __init__(self, fmgip,fmgadom, secure=0):
                self._url='http://'
                if secure:
                    self._url='https://'
                self._url=self._url+fmgip+'/jsonrpc'
                self._httpheaders={"Content-Type": "application/x-www-form-urlencoded", "Accept": "*/*"}
                self._myadom=fmgadom
        ...
        def _parse_chart_cmd(self,obj):
                ...
                try:
                    for e in obj['result'][0]['data']:
                        ...
                        chart_type[e['name']]=(e['graph-type'])
                        chart_subtype[e['name']]=(e['table-subtype'])
                except:
                    pass
                return chart,chart_type,chart_subtype

        def _parse_chart_info(self,obj):
            chart_info={}

            try:

                chart_info['table_subtype']=obj['result'][0]['data']['table-subtype']
                chart_info['graph_type']=obj['result'][0]['data']['graph-type']
        ...
        def _get_chart_list(self,obj):
                ...
                try:
                    for e in obj['result'][0]['data']:
                        ...
                        info_list=[]
                        info_list.append(e['graph-type'])
                        info_list.append(e['table-subtype'])
                        chart_graphic_info[e['name']]=info_list

                except:
                    pass
                return chart_name_list,chart_dataset_list,chart_graphic_info

        def getDeviceInfo(self,session):
                layout_url="dvmdb/adom/"+self._myadom+"/device"
                dev_map = {}
                result_obj={}

                try:
                    result_obj=self._read(layout_url, '', session, '')

                    for adm_pass in result_obj['result'][0]['data']:
                        dev_map[adm_pass['name']]=adm_pass['vdom'][0]['devid']
                except:
                    pass
                return dev_map

        def getVdomList(self,session):
                layout_url="dvmdb/adom/"+self._myadom+"/device"
                adom_list = []
                result_obj={}
                try:
                    ...
                    for data in result_obj['result'][0]['data'][0]['vdom']:
                        adom_list.append(data['name'])
                    adom_list.remove('rootp')
                except:
                    pass
                return adom_list


        def getAdomList(self,session):
            layout_url="dvmdb/adom/"
            adom_list = []
            result_obj={}
            ...
            try:
                ...
                for adom_desc in result_obj['result'][0]['data']:
                    adom_list.append(adom_desc['name']);
                adom_list.remove('rootp')
                adom_list.remove('others')
                adom_list.remove('root')
                adom_list.remove('FortiCarrier')
                adom_list.remove('FortiMail')
                adom_list.remove('FortiCache')
                adom_list.remove('FortiWeb')
                adom_list.remove('SysLog')
                adom_list.remove('FortiClient')
        ...
        def _create_dataset(self,session,name,query,logtype,num):
            ...
            for ch in total_num:
                if (num==1):
                    ...
                    data="[{\"name\": "+"\""+name+str(ch)+"\","+"\"query\":"+"\""+query +"\","+"\"log-type\":"+logtype +","         
        ...
        def _create_chart(self,session,dataset_name,num):
            ...
            for ch in total_template:
               ...
               "\""+ dataset_name+str(ch) +"\"," +"\"resolve-hostname\":"+"0" +","+"\"favorite\":"+"0" +"," \
                ...
                #remove incorrect drill down config for now.
#                data="[{\"name\": \"chart"+str(ch)+"\","+"\"category\":"+"\"traffic"+"\","+"\"description\":"+"\"chart"+str(ch)+"\","+"\"dataset\":"+\
...
#                 "\"y-axis-group-by\":"+"\"group"+"\"," +"\"y-axis-data-top\":"+"10"+","+"\"scale\":"+"1"+"," +"\"protected\":"+"0 "                                                                
        ...
        #        data=data+","+"\"drill-down-table\":["
           ...
           #     data=data+"]"
                ...
                tableColumnObj3="{"+"\"id\""+":" + "3"  +  ","   +"\"header\""+":" + "\"total\""+ "," +" \"data-type\""+":" + "0" +","+" \"data-binding\":"+"\"total\""+"," +" \"column-num\""+":" + "10"+","+ " \"data-top\""+":" + "10"+","+ " \"column-attr\""+":" + "0"+","+ " \"column-icon\""+":" + "0"+"}"
                data=data+tableColumnObj1+","
                data=data+tableColumnObj2+","
                data += tableColumnObj3
                data += "]"
                data += "}]"
        ...
        # def _get_logarray(self,obj):
        #     total_num=range(int(num))
        #     for ch in total_num:
        #         data="[{\"name\": \"FG200B0000000001\",\"vdom\":\"root\"}]"
        #         print "data="+data
        #     try:
        #         result_obj=self._write("dvmb/adom/"+self._myadom+"/group/aa", '', session, data)
        #         if result_obj['result'][0]['status']['code'] == 0 and result_obj['result'][0]['status']['message'] == 'OK':
        #             print "config sucess!"
        #         else:
        #             print "config fail"
        ...
        def _create_logarray(self,session,logarray_name,num):
            total_num=range(int(num))
            for ch in total_num:
               # data="[{\"name\": \"chart"+str(ch)+"\","+"\"category\":"+"\"traffic"+"\","+"\"description\":"+"\"chart"+str(ch)+"\","+"\"dataset\":"+ \
                data="[{\"name\": \"lg"+str(ch)+"\","+"\"old_devices\":"+"\""+"\","+"\"disk_quota\":"+"\"1000"+"\","+"\"disk_full_act\":"+"\"0"+"\","+"\"type\":"+"\"1"+"\","+"\"os_type\":"+"\"0"+"\","+"\"desc\":"+"\"aaa\""
                data += "}]"
                print "data="+data
                try:
                    result_obj=self._write("dvmdb/adom/"+self._myadom+"/log_group/", '', session, data)
                    if result_obj['result'][0]['status']['code'] == 0 and result_obj['result'][0]['status']['message'] == 'OK':
                        print "config sucess!"
                    else:
                        print "config fail"
        ...
        def _add_dev_to_logarray(self,session,logarray_name,num):
            total_num=range(int(num))
            for ch in total_num:
                dev_id=""
                if ch<9:
                    dev_id="FG200B000000000"+str(ch+1);
                if ((ch>=9) and (ch<99)):
                    dev_id="FG200B00000000"+str(ch+1);
                if ((ch>=99) and (ch<999)):
                    dev_id="FG200B000000"+str(ch+1);
                if ((ch>=999) and (ch<9999)):
                    dev_id="FG200B000000"+str(ch+1);
               ...
               # data1="asdfasdfasdf"
                ...
                data1="[{\"name\":" +"\""+dev_id+"\""+","+"\"vdom\":" +"\""+self._myadom+"\""
                data1 += "}]"
                print "data1="+data1
                try:
                    result_obj=self._write("dvmdb/adom/"+self._myadom+"/log_group/"+"lg"+str(ch)+"/object member", '', session, data1)
                    if result_obj['result'][0]['status']['code'] == 0 and result_obj['result'][0]['status']['message'] == 'OK':
                        print "config sucess!"
                    else:
                        print "config fail"
